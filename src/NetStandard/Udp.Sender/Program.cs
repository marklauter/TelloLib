...
namespace Udp.Sender
{
    internal class Program
    {
        ...
        private static void TestRawUdp(int port)
        {
            ...
            {
                var endpoint = new IPEndPoint(IPAddress.Parse("192.168.10.1"), port);
                ...
                while (true)
                {
                    ...
                    timer.Restart();
                    client.Send(datagram, message.Length);
                    //client.BeginReceive(OnReceive, client);

                    while (client.Available == 0 && timer.ElapsedMilliseconds <= 10000)
                    {
                        spinWait.SpinOnce();
                    }
                    ...
                    {
                        Console.WriteLine($"request duration: {ms}ms");
                        var remoteEndpoint = default(IPEndPoint);
                        var responseDatagram = client.Receive(ref remoteEndpoint);
                        var responseMessage = Encoding.UTF8.GetString(responseDatagram);
